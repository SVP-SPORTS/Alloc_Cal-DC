...
import { Text, Input, Grid, Table, Container, Checkbox, Flex, Center, Group, Col, Button, } from '@mantine/core';
  ...
  const onSave = async () => {
    ...
    try {
      for (let i = 0; i < tableData.length; i++) { 
        ...
        const dataToSend = {
          storeName: tableData[i].storeName,
         
        };
  
        const response = await axios.post('http://localhost:5000/api/store', dataToSend);
        console.log(response.data);
      }
    } catch (error) {
    ...
    try {
      // Calculate totalAllocation and overstock for all sizes
      let totalAllocationSum = 0;
      let overstockSum = 0;
  
      for(let i = 0; i < sizes.length; i++) {
        totalAllocationSum += calculateTotalAllocation(i);
        overstockSum += calculateOverstock(i);
      }
  
      const response = await axios.post('http://localhost:5000/api/styleqty/add', {
        supplierName: supplierName,
        poNo : poNo,
        styleNo: styleNo,
        totalAllocation: totalAllocationSum,
        overstock: overstockSum,
        receivedQuantities: sizes.map((size, index) => ({
          size: size,
     ...
     try {
      for (let i = 0; i < tableData.length; i++) { 
        ...
        const dataToSend = {
          ...
          sizeQuantities: tableData[i].sizeQuantities.map(row => ({
            size: row.size,
        ...
        const response = await axios.post('http://localhost:5000/api/tabledata', dataToSend);
        console.log(response.data);
      }
    } catch (error) {
      console.error('Failed to save data to backend:', error);
    ...
    try {
      for (let i = 0; i < tableData.length; i++) { 
        ...
        const response = await axios.post('http://localhost:5000/api/store_data', dataToSend);
        console.log(response.data);
      }
    } catch (error) {
      console.error('Failed to save data to backend:', error);
          ...
          <Table id="table-to-print-1" className="table">
            ...
            <tbody >
              <tr>
                ...
                <td>
                  <Input type="number" value={totalProductionSum} readOnly />
                </td>
                <td />
              </tr>
