...
import { Grid, Table, Paper, Col, Input, Button, Modal, Group} from '@mantine/core';
  ...
  return (
    ...
    <HomePage/>
      <Grid gutter="md" className="gridContainer">
        <Col span={12}>
          <Paper shadow="xs" className="tableContainer">
            <Table className={classes.table}>
              <thead className={classes.tableCell}>
                <tr className="tableHeader">
                  ...
                  <th>Color</th>
                  <th>Cost</th>
                  <th style={{ borderLeft: '1px  solid'}}>MSRP</th>
                </tr>
              </thead>
              <tbody className={classes.tableCell}>
                {Array.isArray(filteredData) && filteredData.map((row, index) => (
                  <tr key={index} className={classes.tableCell}>
                    ...
                    <td>{row.style_no}</td>
                    <td><Input defaultValue={row.description} onBlur={(e) => handleEdit(row.style_no, 'description', e.target.value)} /></td>
                    <td><Input defaultValue={row.color} onBlur={(e) => handleEdit(row.style_no, 'color', e.target.value)} /></td>
                    <td><Input defaultValue={row.cost.toString()} onBlur={(e) => handleEdit(row.style_no, 'cost', e.target.value)} /></td>
                    <td style={{ borderLeft: '1px  solid'}}><Input defaultValue={row.msrp.toString()} onBlur={(e) => handleEdit(row.style_no, 'msrp', e.target.value)} /></td>
                  </tr>
                ))}
              </tbody>
            </Table>
